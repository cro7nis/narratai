...
def generate_from_srt(input_component, speed, language, radio, details, progress=gr.Progress()):
    ...
    if radio == 'Predifined speakers':
        ...
        sr, audio_array = details
        user_wav_file = os.path.join(settings.cache_dir, 'user.wav')
        wavfile.write(user_wav_file, sr, audio_array)
        voice_generator.load_model_if_not_loaded()
        gpt_cond_latent, speaker_embedding = voice_generator.model.get_conditioning_latents(audio_path=user_wav_file)
        audio_filepath = narrator.narrate(raw_subtitles=captions, voice_speed=speed, language=language,
...
with gr.Blocks(
    ...
    gr.Image('assets/image/Narratai_lg.png', show_label=False, show_download_button=False, show_share_button=False,
    ...
    with gr.Tab('Create SRT'):
        ...
        # file_component.clear(clear_state, inputs=state, outputs=state)
        generate_button_2 = gr.Button(value='Generate', variant='primary')
    ...
    with gr.Tab('About'):
        ...
        Narratai enhances media accessibility and localization while showcasing the power of decentralized infrastructure and AI. Find the code in the <a href="https://github.com/cro7nis/narratai/">Github Repo</a>
        ...
        Narratai currently supports media files up to 20MB.
        
        Narratai is not intented for commercial purposes.
        """)
    ...
    gr.HTML(
        f'<div class="footer_main"> \
            <p> \
                Created with ♥️ by <a href="https://x.com/cro7nis">cro7</a> and hosted on Akash Network <img src="{akash_logo_path}" alt="Akash logo" width="20" style="vertical-align: top; display: inline;"> \
            </p> \
        </div>\
        ') \
